...
///
/// 1. **Slot 0 transparency check (warning)**: Slot 0 should match the configured extrinsic transparency color, as it
/// is
///    typically reserved for transparent pixels. If slot 0 contains a different color, a warning is emitted (this may
///    be intentional for .pla blend color usage).
///
...
/// palettes, and palette hints. When the global limit is exceeded, an error is reported showing the pixel that
/// triggered the violation, along with a complete color count summary.
///
/// Additionally, this function warns about unused colors in Porytiles override palettes and palette hints, helping
/// users identify manual color specifications that don't correspond to any colors in the actual input assets.
///
/// @param services Common services parameter store.
/// @param tileset_name The name of the tileset being validated (used for config lookup).
...
///    across all animations. Each key frame tile must be unique so the game engine can properly identify which
///    animation a tile belongs to.
///
