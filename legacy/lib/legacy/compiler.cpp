...
buildColorIndexMaps(const PorytilesContext &ctx, const CompilerMode compilerMode,
    ...
    if (compilerMode == CompilerMode::PRIMARY) {
        size = (PAL_SIZE - 1) * ctx.fieldmapConfig.numPalettesInPrimary;
        if (colorIndex > size) {
            fatalError = true;
        }
        ...
        size = (PAL_SIZE - 1) * ctx.fieldmapConfig.numPalettesTotal;
        if (colorIndex > size) {
            fatalError = true;
        }
    } else {
...
static void assignTilesSecondary(PorytilesContext &ctx, CompiledTileset &compiled,
    ...
    for (const auto &indexedNormTile : indexedNormTilesWithColorSets) {
        ...
        auto it = std::ranges::find_if(allColorSets, [&colorSet](const auto &assignedPal) {
            // Find which of the allColorSets palettes this tile belongs to
            return (colorSet.first & ~assignedPal.first).none();
        });
        if (it == std::end(allColorSets)) {
...
compile(PorytilesContext &ctx, CompilerMode compilerMode, const DecompiledTileset &decompiledTileset,
    ...
    if (compilerMode == CompilerMode::PRIMARY) {
        ...
        if (std::size_t srcMetatileCount = (decompiledTileset.tiles.size() / ctx.fieldmapConfig.numTilesPerMetatile);
...
TEST_CASE("insertRGBA should add new colors in order and return the correct index for a given color") {
    ...
    porytiles_legacy::NormalizedPalette palette1{};
    palette1.size = 1;
    palette1.colors = {};

    porytiles_legacy::RGBATile dummy{};
    dummy.type = porytiles_legacy::TileType::LAYERED;
    dummy.metatileIndex = 0;
    dummy.subtile = porytiles_legacy::Subtile::NORTHEAST;

    // Transparent should return 0
    CHECK(insertRGBA(ctx, porytiles_legacy::CompilerMode::PRIMARY, dummy, ctx.compilerConfig.transparencyColor, palette1,
                     porytiles_legacy::RGBA_MAGENTA, 0, 0, true) == 0);
    CHECK(insertRGBA(ctx, porytiles_legacy::CompilerMode::PRIMARY, dummy, ctx.compilerConfig.transparencyColor, palette1,
                     porytiles_legacy::RGBA32{0, 0, 0, porytiles_legacy::ALPHA_TRANSPARENT}, 0, 0, true) == 0);

    // insert colors
    CHECK(insertRGBA(ctx, porytiles_legacy::CompilerMode::PRIMARY, dummy, ctx.compilerConfig.transparencyColor, palette1,
                     porytiles_legacy::RGBA32{0, 0, 0, porytiles_legacy::ALPHA_OPAQUE}, 0, 0, true) == 1);
    CHECK(insertRGBA(ctx, porytiles_legacy::CompilerMode::PRIMARY, dummy, ctx.compilerConfig.transparencyColor, palette1,
                     porytiles_legacy::RGBA32{8, 0, 0, porytiles_legacy::ALPHA_OPAQUE}, 0, 0, true) == 2);
    CHECK(insertRGBA(ctx, porytiles_legacy::CompilerMode::PRIMARY, dummy, ctx.compilerConfig.transparencyColor, palette1,
                     porytiles_legacy::RGBA32{16, 0, 0, porytiles_legacy::ALPHA_OPAQUE}, 0, 0, true) == 3);
    CHECK(insertRGBA(ctx, porytiles_legacy::CompilerMode::PRIMARY, dummy, ctx.compilerConfig.transparencyColor, palette1,
                     porytiles_legacy::RGBA32{24, 0, 0, porytiles_legacy::ALPHA_OPAQUE}, 0, 0, true) == 4);
    CHECK(insertRGBA(ctx, porytiles_legacy::CompilerMode::PRIMARY, dummy, ctx.compilerConfig.transparencyColor, palette1,
                     porytiles_legacy::RGBA32{32, 0, 0, porytiles_legacy::ALPHA_OPAQUE}, 0, 0, true) == 5);
    CHECK(insertRGBA(ctx, porytiles_legacy::CompilerMode::PRIMARY, dummy, ctx.compilerConfig.transparencyColor, palette1,
                     porytiles_legacy::RGBA32{40, 0, 0, porytiles_legacy::ALPHA_OPAQUE}, 0, 0, true) == 6);
    CHECK(insertRGBA(ctx, porytiles_legacy::CompilerMode::PRIMARY, dummy, ctx.compilerConfig.transparencyColor, palette1,
                     porytiles_legacy::RGBA32{48, 0, 0, porytiles_legacy::ALPHA_OPAQUE}, 0, 0, true) == 7);
    CHECK(insertRGBA(ctx, porytiles_legacy::CompilerMode::PRIMARY, dummy, ctx.compilerConfig.transparencyColor, palette1,
                     porytiles_legacy::RGBA32{56, 0, 0, porytiles_legacy::ALPHA_OPAQUE}, 0, 0, true) == 8);
    CHECK(insertRGBA(ctx, porytiles_legacy::CompilerMode::PRIMARY, dummy, ctx.compilerConfig.transparencyColor, palette1,
                     porytiles_legacy::RGBA32{64, 0, 0, porytiles_legacy::ALPHA_OPAQUE}, 0, 0, true) == 9);
    CHECK(insertRGBA(ctx, porytiles_legacy::CompilerMode::PRIMARY, dummy, ctx.compilerConfig.transparencyColor, palette1,
                     porytiles_legacy::RGBA32{72, 0, 0, porytiles_legacy::ALPHA_OPAQUE}, 0, 0, true) == 10);
    CHECK(insertRGBA(ctx, porytiles_legacy::CompilerMode::PRIMARY, dummy, ctx.compilerConfig.transparencyColor, palette1,
                     porytiles_legacy::RGBA32{80, 0, 0, porytiles_legacy::ALPHA_OPAQUE}, 0, 0, true) == 11);
    CHECK(insertRGBA(ctx, porytiles_legacy::CompilerMode::PRIMARY, dummy, ctx.compilerConfig.transparencyColor, palette1,
                     porytiles_legacy::RGBA32{88, 0, 0, porytiles_legacy::ALPHA_OPAQUE}, 0, 0, true) == 12);
    CHECK(insertRGBA(ctx, porytiles_legacy::CompilerMode::PRIMARY, dummy, ctx.compilerConfig.transparencyColor, palette1,
                     porytiles_legacy::RGBA32{96, 0, 0, porytiles_legacy::ALPHA_OPAQUE}, 0, 0, true) == 13);
    CHECK(insertRGBA(ctx, porytiles_legacy::CompilerMode::PRIMARY, dummy, ctx.compilerConfig.transparencyColor, palette1,
                     porytiles_legacy::RGBA32{104, 0, 0, porytiles_legacy::ALPHA_OPAQUE}, 0, 0, true) == 14);
    CHECK(insertRGBA(ctx, porytiles_legacy::CompilerMode::PRIMARY, dummy, ctx.compilerConfig.transparencyColor, palette1,
                     porytiles_legacy::RGBA32{112, 0, 0, porytiles_legacy::ALPHA_OPAQUE}, 0, 0, true) == 15);
    ...
    insertRGBA(ctx, porytiles_legacy::CompilerMode::PRIMARY, dummy, ctx.compilerConfig.transparencyColor, palette1,
               porytiles_legacy::RGBA_CYAN, 0, 0, true);
    ...
    insertRGBA(ctx, porytiles_legacy::CompilerMode::PRIMARY, dummy, ctx.compilerConfig.transparencyColor, palette1,
               porytiles_legacy::RGBA32{0, 0, 0, 12}, 0, 0, true);
...
TEST_CASE("candidate should return the NormalizedTile with requested flips") {
    ...
    porytiles_legacy::RGBATile tile = tiles.tiles[0];

    SUBCASE("case: no flips") {
        std::vector<porytiles_legacy::RGBATile> singleFrameTile = {tile};
        porytiles_legacy::NormalizedTile candidate =
            porytiles_legacy::candidate(ctx, porytiles_legacy::CompilerMode::PRIMARY, ctx.compilerConfig.transparencyColor,
                                  singleFrameTile, false, false, true);
        CHECK(candidate.palette.size == 9);
        CHECK(candidate.palette.colors[0] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_MAGENTA));
        CHECK(candidate.palette.colors[1] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_RED));
        CHECK(candidate.palette.colors[2] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_YELLOW));
        CHECK(candidate.palette.colors[3] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_GREEN));
        CHECK(candidate.palette.colors[4] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_WHITE));
        CHECK(candidate.palette.colors[5] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_BLUE));
        CHECK(candidate.palette.colors[6] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_BLACK));
        CHECK(candidate.palette.colors[7] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_CYAN));
        CHECK(candidate.palette.colors[8] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_GREY));
    ...
    SUBCASE("case: hFlip") {
        ...
        CHECK(candidate.palette.size == 9);
        CHECK(candidate.palette.colors[0] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_MAGENTA));
        CHECK(candidate.palette.colors[1] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_YELLOW));
        CHECK(candidate.palette.colors[2] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_RED));
        CHECK(candidate.palette.colors[3] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_WHITE));
        CHECK(candidate.palette.colors[4] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_GREEN));
        CHECK(candidate.palette.colors[5] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_BLUE));
        CHECK(candidate.palette.colors[6] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_CYAN));
        CHECK(candidate.palette.colors[7] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_BLACK));
        CHECK(candidate.palette.colors[8] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_GREY));
        CHECK(candidate.keyFrame().colorIndexes[0] == 1);
        CHECK(candidate.keyFrame().colorIndexes[7] == 2);
        CHECK(candidate.keyFrame().colorIndexes[9] == 3);
    ...
    SUBCASE("case: vFlip") {
        ...
        CHECK(candidate.palette.size == 9);
        CHECK(candidate.palette.colors[0] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_MAGENTA));
        CHECK(candidate.palette.colors[1] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_GREY));
        CHECK(candidate.palette.colors[2] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_BLUE));
        CHECK(candidate.palette.colors[3] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_BLACK));
        CHECK(candidate.palette.colors[4] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_CYAN));
        CHECK(candidate.palette.colors[5] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_GREEN));
        CHECK(candidate.palette.colors[6] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_RED));
        CHECK(candidate.palette.colors[7] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_YELLOW));
        CHECK(candidate.palette.colors[8] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_WHITE));
        CHECK(candidate.keyFrame().colorIndexes[0] == 1);
        CHECK(candidate.keyFrame().colorIndexes[7] == 2);
        CHECK(candidate.keyFrame().colorIndexes[9] == 3);
        CHECK(candidate.keyFrame().colorIndexes[14] == 4);
    ...
    SUBCASE("case: hFlip and vFlip") {
        ...
        CHECK(candidate.palette.size == 9);
        CHECK(candidate.palette.colors[0] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_MAGENTA));
        CHECK(candidate.palette.colors[1] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_BLUE));
        CHECK(candidate.palette.colors[2] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_GREY));
        CHECK(candidate.palette.colors[3] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_CYAN));
        CHECK(candidate.palette.colors[4] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_BLACK));
        CHECK(candidate.palette.colors[5] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_RED));
        CHECK(candidate.palette.colors[6] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_GREEN));
        CHECK(candidate.palette.colors[7] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_YELLOW));
        CHECK(candidate.palette.colors[8] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_WHITE));
        CHECK(candidate.keyFrame().colorIndexes[0] == 1);
        CHECK(candidate.keyFrame().colorIndexes[7] == 2);
...
TEST_CASE("normalize should return the normal form of the given tile") {
    ...
    std::vector<porytiles_legacy::RGBATile> singleFrameTile = {tile};
    porytiles_legacy::NormalizedTile normalizedTile =
        porytiles_legacy::normalize(ctx, porytiles_legacy::CompilerMode::PRIMARY, singleFrameTile);
...
TEST_CASE("normalizeDecompTiles should correctly normalize all tiles in the decomp tileset") {
    ...
    auto [indexedNormTiles, _1, _2] = normalizeDecompTiles(ctx, porytiles_legacy::CompilerMode::PRIMARY, tiles, {}, {});
    ...
    CHECK(indexedNormTiles[0].second.palette.colors[0] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_MAGENTA));
    CHECK(indexedNormTiles[0].second.palette.colors[1] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_BLUE));
    ...
    CHECK(indexedNormTiles[1].second.palette.colors[0] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_MAGENTA));
    CHECK(indexedNormTiles[1].second.palette.colors[1] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_GREEN));
    CHECK(indexedNormTiles[1].second.palette.colors[2] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_RED));
    ...
    CHECK(indexedNormTiles[2].second.palette.colors[0] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_MAGENTA));
    CHECK(indexedNormTiles[2].second.palette.colors[1] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_CYAN));
    CHECK(indexedNormTiles[2].second.palette.colors[2] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_GREEN));
    ...
    CHECK(indexedNormTiles[3].second.palette.colors[0] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_MAGENTA));
    CHECK(indexedNormTiles[3].second.palette.colors[1] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_BLUE));
...
TEST_CASE("normalizeDecompTiles should correctly normalize multi-frame animated tiles") {
    ...
    porytiles_legacy::DecompiledTileset tiles =
        porytiles_legacy::importTilesFromPng(ctx, porytiles_legacy::CompilerMode::PRIMARY, tilesPng);
    ...
    porytiles_legacy::AnimationPng<png::rgba_pixel> water00{
        png::image<png::rgba_pixel>{"resources/doctests/anim_water_1/00.png"}, "anim_water_1", "00.png"};
    porytiles_legacy::AnimationPng<png::rgba_pixel> water01{
    ...
    std::vector<porytiles_legacy::AnimationPng<png::rgba_pixel>> waterAnim{};
...
TEST_CASE("buildColorIndexMaps should build a map of all unique colors in the decomp tileset") {
    ...
    auto [indexedNormTiles, _1, _2] =
        porytiles_legacy::normalizeDecompTiles(ctx, porytiles_legacy::CompilerMode::PRIMARY, tiles, {}, {});

    auto [colorToIndex, indexToColor] =
        porytiles_legacy::buildColorIndexMaps(ctx, porytiles_legacy::CompilerMode::PRIMARY, indexedNormTiles, {}, {}, {});

    CHECK(colorToIndex.size() == 4);
    CHECK(colorToIndex[porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_BLUE)] == 0);
    CHECK(colorToIndex[porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_GREEN)] == 1);
    CHECK(colorToIndex[porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_RED)] == 2);
    CHECK(colorToIndex[porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_CYAN)] == 3);
}

TEST_CASE("toColorSet should return the correct bitset based on the supplied palette") {
    std::unordered_map<porytiles_legacy::BGR15, std::size_t> colorIndexMap = {
        {porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_BLUE), 0},   {porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_RED), 1},
        {porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_GREEN), 2},  {porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_CYAN), 3},
        {porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_YELLOW), 4},
    };

    SUBCASE("palette 1") {
        porytiles_legacy::NormalizedTile tile{porytiles_legacy::RGBA_MAGENTA};
        tile.palette.size = 2;
        tile.palette.colors[0] = porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_MAGENTA);
        tile.palette.colors[1] = porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_RED);

        ColorSet colorSet = porytiles_legacy::toColorSet(colorIndexMap, tile);
    ...
    SUBCASE("palette 2") {
        ...
        tile.palette.size = 4;
        tile.palette.colors[0] = porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_MAGENTA);
        tile.palette.colors[1] = porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_YELLOW);
        tile.palette.colors[2] = porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_GREEN);
        tile.palette.colors[3] = porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_CYAN);
...
TEST_CASE("matchNormalizedWithColorSets should return the expected data structures") {
    ...
    auto [indexedNormTilesWithColorSets, colorSets, _3, _4] =
        porytiles_legacy::matchNormalizedWithColorSets(colorToIndex, indexedNormTiles, {}, {});
    ...
    CHECK(std::get<1>(indexedNormTilesWithColorSets[0]).palette.colors[0] ==
          porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_MAGENTA));
    CHECK(std::get<1>(indexedNormTilesWithColorSets[0]).palette.colors[1] ==
          porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_BLUE));
    ...
    CHECK(std::get<1>(indexedNormTilesWithColorSets[1]).palette.colors[1] ==
          porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_GREEN));
    CHECK(std::get<1>(indexedNormTilesWithColorSets[1]).palette.colors[2] ==
          porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_RED));
    ...
    CHECK(std::get<1>(indexedNormTilesWithColorSets[2]).palette.colors[1] ==
          porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_CYAN));
...
TEST_CASE("assign should correctly assign all normalized palettes or fail if impossible") {
    SUBCASE("It should successfully allocate a simple 2x2 tileset png") {
        ...
        png::image<png::rgba_pixel> png1{"resources/doctests/2x2_pattern_2.png"};
        porytiles_legacy::DecompiledTileset tiles =
            porytiles_legacy::importTilesFromPng(ctx, porytiles_legacy::CompilerMode::PRIMARY, png1);
        auto [indexedNormTiles, _1, _2] =
            porytiles_legacy::normalizeDecompTiles(ctx, porytiles_legacy::CompilerMode::PRIMARY, tiles, {}, {});
        auto [colorToIndex, indexToColor] =
            porytiles_legacy::buildColorIndexMaps(ctx, porytiles_legacy::CompilerMode::PRIMARY, indexedNormTiles, {}, {}, {});
        ...
        porytiles_legacy::AssignState state = {hardwarePalettes, unassigned.size(), 0};
...
TEST_CASE("makeTile should create the expected GBATile from the given NormalizedTile and GBAPalette") {
    porytiles_legacy::PorytilesContext ctx{};
    ctx.compilerConfig.transparencyColor = porytiles_legacy::RGBA_MAGENTA;
    ...
    porytiles_legacy::DecompiledTileset tiles = porytiles_legacy::importTilesFromPng(ctx, porytiles_legacy::CompilerMode::PRIMARY, png1);
    auto [indexedNormTiles, _1, _2] = normalizeDecompTiles(ctx, porytiles_legacy::CompilerMode::PRIMARY, tiles, {}, {});
    auto compiledTiles = porytiles_legacy::compile(ctx, porytiles_legacy::CompilerMode::PRIMARY, tiles, {}, {}, {});

    porytiles_legacy::GBATile tile0 = porytiles_legacy::makeTile(
        indexedNormTiles[0].second, porytiles_legacy::NormalizedTile::keyFrameIndex(), compiledTiles->palettes[0]);
    CHECK_FALSE(indexedNormTiles[0].second.hFlip);
    CHECK(indexedNormTiles[0].second.vFlip);
    ...
    porytiles_legacy::GBATile tile1 = porytiles_legacy::makeTile(
        indexedNormTiles[1].second, porytiles_legacy::NormalizedTile::keyFrameIndex(), compiledTiles->palettes[1]);
    CHECK_FALSE(indexedNormTiles[1].second.hFlip);
    CHECK_FALSE(indexedNormTiles[1].second.vFlip);
    ...
    porytiles_legacy::GBATile tile2 = porytiles_legacy::makeTile(
        indexedNormTiles[2].second, porytiles_legacy::NormalizedTile::keyFrameIndex(), compiledTiles->palettes[1]);
    CHECK(indexedNormTiles[2].second.hFlip);
    CHECK_FALSE(indexedNormTiles[2].second.vFlip);
    ...
    porytiles_legacy::GBATile tile3 = porytiles_legacy::makeTile(
        indexedNormTiles[3].second, porytiles_legacy::NormalizedTile::keyFrameIndex(), compiledTiles->palettes[0]);
    CHECK(indexedNormTiles[3].second.hFlip);
    CHECK(indexedNormTiles[3].second.vFlip);
...
TEST_CASE("compile simple example should perform as expected") {
    ...
    CHECK(compiledTiles->palettes.at(0).colors[0] == porytiles_legacy::rgbaToBgr(ctx.compilerConfig.transparencyColor));
    CHECK(compiledTiles->palettes.at(0).colors[1] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_BLUE));
    CHECK(compiledTiles->palettes.at(1).colors[0] == porytiles_legacy::rgbaToBgr(ctx.compilerConfig.transparencyColor));
    CHECK(compiledTiles->palettes.at(1).colors[1] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_GREEN));
    CHECK(compiledTiles->palettes.at(1).colors[2] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_RED));
    CHECK(compiledTiles->palettes.at(1).colors[3] == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_CYAN));
     ...
     */
    porytiles_legacy::GBATile &tile0 = compiledTiles->tiles[0];
    for (size_t i = 0; i < 64; i++) {
        CHECK(tile0.colorIndexes[i] == 0);
    }

    porytiles_legacy::GBATile &tile1 = compiledTiles->tiles[1];
    CHECK(tile1.colorIndexes[0] == 0);
    ...
    porytiles_legacy::GBATile tile2 = compiledTiles->tiles[2];
    CHECK(tile2.colorIndexes[0] == 0);
    ...
    porytiles_legacy::GBATile tile3 = compiledTiles->tiles[3];
    CHECK(tile3.colorIndexes[0] == 0);
...
TEST_CASE("compile function should fill out primary CompiledTileset struct with expected values") {
    ...
    porytiles_legacy::DecompiledTileset decompiledPrimary = porytiles_legacy::importLayeredTilesFromPngs(
        ctx, porytiles_legacy::CompilerMode::PRIMARY, std::unordered_map<std::size_t, porytiles_legacy::Attributes>{},
        bottomPrimary, middlePrimary, topPrimary);

    auto compiledPrimary = porytiles_legacy::compile(ctx, porytiles_legacy::CompilerMode::PRIMARY, decompiledPrimary, {}, {}, {});
    ...
    for (std::size_t tileIndex = 0; tileIndex < compiledPrimary->tiles.size(); tileIndex++) {
        for (std::size_t row = 0; row < porytiles_legacy::TILE_SIDE_LENGTH_PIX; row++) {
            for (std::size_t col = 0; col < porytiles_legacy::TILE_SIDE_LENGTH_PIX; col++) {
                CHECK(compiledPrimary->tiles[tileIndex].colorIndexes[col + (row * porytiles_legacy::TILE_SIDE_LENGTH_PIX)] ==
                      expectedPng[row][col + (tileIndex * porytiles_legacy::TILE_SIDE_LENGTH_PIX)]);
            }
        }
    }
    ...
    CHECK(compiledPrimary->metatileEntries.size() ==
          porytiles_legacy::METATILES_IN_ROW * ctx.fieldmapConfig.numTilesPerMetatile);
    ...
    CHECK(compiledPrimary->colorIndexMap[porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_RED)] == 0);
    CHECK(compiledPrimary->colorIndexMap[porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_YELLOW)] == 1);
    CHECK(compiledPrimary->colorIndexMap[porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_GREEN)] == 2);
    CHECK(compiledPrimary->colorIndexMap[porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_BLUE)] == 3);
    CHECK(compiledPrimary->colorIndexMap[porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_WHITE)] == 4);
...
TEST_CASE("compile function should fill out secondary CompiledTileset struct with expected values") {
    ...
    ctx.compilerContext.pairedPrimaryTileset =
        porytiles_legacy::compile(ctx, porytiles_legacy::CompilerMode::PRIMARY, decompiledPrimary, {}, {}, {});
    ...
    porytiles_legacy::DecompiledTileset decompiledSecondary = porytiles_legacy::importLayeredTilesFromPngs(
        ctx, porytiles_legacy::CompilerMode::SECONDARY, std::unordered_map<std::size_t, porytiles_legacy::Attributes>{},
        bottomSecondary, middleSecondary, topSecondary);
    auto compiledSecondary =
        porytiles_legacy::compile(ctx, porytiles_legacy::CompilerMode::SECONDARY, decompiledSecondary, {}, {}, {});
    ...
    for (std::size_t tileIndex = 0; tileIndex < compiledSecondary->tiles.size(); tileIndex++) {
        for (std::size_t row = 0; row < porytiles_legacy::TILE_SIDE_LENGTH_PIX; row++) {
            for (std::size_t col = 0; col < porytiles_legacy::TILE_SIDE_LENGTH_PIX; col++) {
                CHECK(
                    compiledSecondary->tiles[tileIndex].colorIndexes[col + (row * porytiles_legacy::TILE_SIDE_LENGTH_PIX)] ==
                    expectedPng[row][col + (tileIndex * porytiles_legacy::TILE_SIDE_LENGTH_PIX)]);
            }
        }
    }
    ...
    // Check that colorIndexMap is correct
    CHECK(compiledSecondary->colorIndexMap[porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_RED)] == 0);
    CHECK(compiledSecondary->colorIndexMap[porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_YELLOW)] == 1);
    CHECK(compiledSecondary->colorIndexMap[porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_GREEN)] == 2);
    CHECK(compiledSecondary->colorIndexMap[porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_BLUE)] == 3);
    CHECK(compiledSecondary->colorIndexMap[porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_WHITE)] == 4);
    CHECK(compiledSecondary->colorIndexMap[porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_CYAN)] == 5);
    CHECK(compiledSecondary->colorIndexMap[porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_PURPLE)] == 6);
    CHECK(compiledSecondary->colorIndexMap[porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_LIME)] == 7);
    CHECK(compiledSecondary->colorIndexMap[porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_GREY)] == 8);
...
TEST_CASE("compile function should correctly compile primary set with animated tiles") {
    ...
    std::vector<porytiles_legacy::AnimationPng<png::rgba_pixel>> flowerWhiteAnim{};
    std::vector<porytiles_legacy::AnimationPng<png::rgba_pixel>> waterAnim{};
    ...
    porytiles_legacy::importAnimTiles(ctx, porytiles_legacy::CompilerMode::PRIMARY, anims, decompiledPrimary);
...
TEST_CASE("compile function should correctly compile secondary set with animated tiles") {
    ...
    std::vector<std::vector<porytiles_legacy::AnimationPng<png::rgba_pixel>>> animsSecondary{};
    animsSecondary.push_back(flowerRedAnim);

    porytiles_legacy::importAnimTiles(ctx, porytiles_legacy::CompilerMode::SECONDARY, animsSecondary, decompiledSecondary);
...
TEST_CASE("primer tiles should change output of primary compile function") {
    ...
    porytiles_legacy::DecompiledTileset decompiled = porytiles_legacy::importLayeredTilesFromPngs(
        ctx, porytiles_legacy::CompilerMode::PRIMARY, std::unordered_map<std::size_t, porytiles_legacy::Attributes>{},
        bottomPrimary, middlePrimary, topPrimary);
    ...
    std::ifstream primerIfstream{
        std::filesystem::path{"resources/doctests/palette_primer_1/palette-primers/primer.pal"}};
    porytiles_legacy::RGBATile primerTile =
        porytiles_legacy::importPalettePrimer(ctx, porytiles_legacy::CompilerMode::PRIMARY, primerIfstream,
    ...
    std::vector<porytiles_legacy::RGBATile> palettePrimers{};
    palettePrimers.push_back(primerTile);
    primerIfstream.close();

    // Compile with no primer
    auto compiledNoPrimer = porytiles_legacy::compile(ctx, porytiles_legacy::CompilerMode::PRIMARY, decompiled, {}, {}, {});

    // Confirm compiled no primer is as expected
    CHECK(compiledNoPrimer->palettes.at(0).colors.at(0) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{255, 0, 255}));
    CHECK(compiledNoPrimer->palettes.at(0).colors.at(1) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{255, 255, 0}));
    CHECK(compiledNoPrimer->palettes.at(0).colors.at(2) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{255, 0, 0}));
    CHECK(compiledNoPrimer->palettes.at(0).colors.at(3) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{0, 0, 0}));
    CHECK(compiledNoPrimer->palettes.at(1).colors.at(0) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{255, 0, 255}));
    CHECK(compiledNoPrimer->palettes.at(1).colors.at(1) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{0, 255, 255}));
    CHECK(compiledNoPrimer->palettes.at(1).colors.at(2) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{0, 255, 0}));
    CHECK(compiledNoPrimer->palettes.at(1).colors.at(3) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{0, 0, 0}));
    CHECK(compiledNoPrimer->palettes.at(2).colors.at(0) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{255, 0, 255}));
    CHECK(compiledNoPrimer->palettes.at(2).colors.at(1) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{0, 0, 0}));
    CHECK(compiledNoPrimer->palettes.at(2).colors.at(2) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{0, 0, 255}));
    CHECK(compiledNoPrimer->palettes.at(2).colors.at(3) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{0, 0, 0}));
    CHECK(compiledNoPrimer->palettes.at(3).colors.at(0) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{255, 0, 255}));
    CHECK(compiledNoPrimer->palettes.at(3).colors.at(1) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{128, 128, 128}));
    CHECK(compiledNoPrimer->palettes.at(3).colors.at(2) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{255, 255, 255}));
    CHECK(compiledNoPrimer->palettes.at(3).colors.at(3) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{0, 0, 0}));
...
TEST_CASE("overrides should change output of primary compile function") {
    ...
    auto [overrideTile00, overriddenPalSlots00] =
        porytiles_legacy::importPaletteOverride(ctx, porytiles_legacy::CompilerMode::PRIMARY, override00IfStream,
    ...
    auto [overrideTile01, overriddenPalSlots01] =
        porytiles_legacy::importPaletteOverride(ctx, porytiles_legacy::CompilerMode::PRIMARY, override01IfStream,
    ...
    auto [overrideTile02, overriddenPalSlots02] =
        porytiles_legacy::importPaletteOverride(ctx, porytiles_legacy::CompilerMode::PRIMARY, override02IfStream,
    ...
    std::vector<porytiles_legacy::RGBATile> paletteOverrides{};
    ...
    std::unordered_map<size_t, std::vector<std::pair<size_t, porytiles_legacy::BGR15>>> palOverridesMap{};
    ...
    auto compiledOverrides = porytiles_legacy::compile(ctx, porytiles_legacy::CompilerMode::PRIMARY, decompiledPrimary, {},
    ...
    CHECK(compiledOverrides->palettes.at(0).colors.at(0) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{255, 0, 255}));
    CHECK(compiledOverrides->palettes.at(0).colors.at(1) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{96, 96, 120}));
    CHECK(compiledOverrides->palettes.at(0).colors.at(2) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{248, 184, 128}));
    CHECK(compiledOverrides->palettes.at(0).colors.at(3) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{200, 64, 80}));
    CHECK(compiledOverrides->palettes.at(0).colors.at(4) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{232, 144, 112}));

    CHECK(compiledOverrides->palettes.at(1).colors.at(0) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{255, 0, 255}));
    CHECK(compiledOverrides->palettes.at(1).colors.at(1) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{96, 96, 120}));
    CHECK(compiledOverrides->palettes.at(1).colors.at(2) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{112, 184, 240}));
    CHECK(compiledOverrides->palettes.at(1).colors.at(3) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{72, 112, 168}));
    CHECK(compiledOverrides->palettes.at(1).colors.at(4) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{96, 160, 216}));
     ...
     * an override
     */
    CHECK(compiledOverrides->palettes.at(2).colors.at(0) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{255, 0, 255}));
    CHECK(compiledOverrides->palettes.at(2).colors.at(1) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{112, 192, 160}));
    CHECK(compiledOverrides->palettes.at(2).colors.at(2) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{64, 176, 128}));
    CHECK(compiledOverrides->palettes.at(2).colors.at(3) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{160, 208, 192}));
    CHECK(compiledOverrides->palettes.at(2).colors.at(12) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{56, 136, 48}));
    CHECK(compiledOverrides->palettes.at(2).colors.at(13) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{176, 248, 128}));
    CHECK(compiledOverrides->palettes.at(2).colors.at(14) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{56, 80, 0}));
    CHECK(compiledOverrides->palettes.at(2).colors.at(15) == porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{128, 192, 96}));
}
...
TEST_CASE("overrides should change output of secondary compile function") {
    ...
    std::vector<porytiles_legacy::RGBATile> paletteOverridesPrimary{};
    ...
    override01IfStream.close();
    override02IfStream.close();
    std::unordered_map<size_t, std::vector<std::pair<size_t, porytiles_legacy::BGR15>>> palOverridesMapPrimary{};
    ...
    auto [overrideTile03, overriddenPalSlots03] =
        porytiles_legacy::importPaletteOverride(ctx, porytiles_legacy::CompilerMode::SECONDARY, override03IfStream,
    ...
    auto [overrideTile04, overriddenPalSlots04] =
        porytiles_legacy::importPaletteOverride(ctx, porytiles_legacy::CompilerMode::SECONDARY, override04IfStream,
    ...
    auto [overrideTile05, overriddenPalSlots05] =
        porytiles_legacy::importPaletteOverride(ctx, porytiles_legacy::CompilerMode::SECONDARY, override05IfStream,
    ...
    std::vector<porytiles_legacy::RGBATile> paletteOverridesSecondary{};
    ...
    std::unordered_map<size_t, std::vector<std::pair<size_t, porytiles_legacy::BGR15>>> palOverridesMapSecondary{};
    ...
    CHECK(compiledSecondaryOverrides->palettes.at(3).colors.at(4) ==
          porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{80, 48, 56}));
    ...
    CHECK(compiledSecondaryOverrides->palettes.at(3).colors.at(15) ==
          porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{128, 208, 40}));
    ...
    CHECK(compiledSecondaryOverrides->palettes.at(4).colors.at(5) ==
          porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{64, 136, 80}));
    CHECK(compiledSecondaryOverrides->palettes.at(4).colors.at(6) ==
          porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{128, 224, 64}));
    CHECK(compiledSecondaryOverrides->palettes.at(4).colors.at(7) ==
          porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{96, 192, 80}));
    CHECK(compiledSecondaryOverrides->palettes.at(4).colors.at(8) ==
          porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{56, 168, 128}));
    CHECK(compiledSecondaryOverrides->palettes.at(4).colors.at(9) ==
          porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{48, 136, 40}));
    CHECK(compiledSecondaryOverrides->palettes.at(4).colors.at(10) ==
          porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{232, 224, 160}));
    ...
    CHECK(compiledSecondaryOverrides->palettes.at(4).colors.at(13) ==
          porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{144, 112, 64}));
    ...
    CHECK(compiledSecondaryOverrides->palettes.at(5).colors.at(10) ==
          porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{168, 184, 248}));
    CHECK(compiledSecondaryOverrides->palettes.at(5).colors.at(11) ==
          porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{96, 48, 168}));
    CHECK(compiledSecondaryOverrides->palettes.at(5).colors.at(12) ==
          porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{104, 128, 224}));
    CHECK(compiledSecondaryOverrides->palettes.at(5).colors.at(13) ==
          porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA32{168, 104, 152}));
}
#endif // DOCTEST_CONFIG_DISABLE
