...
std::unique_ptr<DecompiledTileset> decompile(PorytilesContext &ctx, DecompilerMode mode,
     ...
     * attribute count (i.e. the true metatile count). If division by 8 matches, then we are dual layer. If 12 matches,
     * we are triple. Otherwise, we have corruption and should fail.
     */
...
TEST_CASE("decompile should decompile a basic primary tileset") {
    ...
    auto decompiledViaAlgorithm =
        porytiles_legacy::decompile(ctx, porytiles_legacy::DecompilerMode::PRIMARY, *compiledPrimary,
...
TEST_CASE("decompile should decompile a basic secondary tileset") {
    ...
    auto decompiledViaAlgorithm =
        porytiles_legacy::decompile(ctx, porytiles_legacy::DecompilerMode::SECONDARY, *compiledSecondary,
