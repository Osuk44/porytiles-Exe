...
void emitAttributes(const PorytilesContext &ctx, std::ostream &out,
    ...
    for (std::size_t i = 0; i < tileset.metatileEntries.size(); i += delta) {
        ...
        if (ctx.targetBaseGame == TargetBaseGame::RUBY || ctx.targetBaseGame == TargetBaseGame::EMERALD) {
            ...
            pt_logln(ctx, stderr,
                     "emitted {}-format metatile {} attribute: [ behavior={}, encounterType={}, terrainType={}, "
                     "layerType={} ]",
                     targetBaseGameString(ctx.targetBaseGame), i / delta, behaviorString,
    ...
    if (tileset.tripleLayer) {
        for (std::size_t metatileIndex = 0; metatileIndex < attributesMap.size(); metatileIndex++) {
            ...
            for (std::size_t subtileIndex = 0; subtileIndex < 12; subtileIndex++) {
                ...
                for (std::size_t pixelIndex = 0; pixelIndex < TILE_NUM_PIX; pixelIndex++) {
                    std::size_t pixelRow = (metatileRow * METATILE_SIDE_LENGTH) +
                                           (layerTileRow * TILE_SIDE_LENGTH_PIX) + (pixelIndex / TILE_SIDE_LENGTH_PIX);
        ...
        if (ctx.targetBaseGame == TargetBaseGame::FIRERED) {
            if (behaviorReverseMap.contains(attributesMap.at(metatileIndex).metatileBehavior)) {
                outCsv << metatileIndex << ","
                       << behaviorReverseMap.at(attributesMap.at(metatileIndex).metatileBehavior) << ","
...
TEST_CASE("emitPalette should write the expected JASC pal to the output stream") {
    ...
    porytiles_legacy::GBAPalette palette{};
    palette.colors[0] = porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_MAGENTA);
    palette.colors[1] = porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_RED);
    palette.colors[2] = porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_GREEN);
    palette.colors[3] = porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_BLUE);
    palette.colors[4] = porytiles_legacy::rgbaToBgr(porytiles_legacy::RGBA_WHITE);
    ...
    porytiles_legacy::emitPalette(ctx, outputStream, palette);
...
TEST_CASE("emitZeroedPalette should write the expected JASC pal to the output stream") {
    ...
    std::stringstream outputStream;
    porytiles_legacy::emitZeroedPalette(ctx, outputStream);
...
TEST_CASE("emitTilesPng should emit the expected tiles.png file") {
    ...
    const size_t imageWidth = porytiles_legacy::TILE_SIDE_LENGTH_PIX * porytiles_legacy::TILES_PNG_WIDTH_IN_TILES;
    const size_t imageHeight =
        porytiles_legacy::TILE_SIDE_LENGTH_PIX * ((compiledPrimary->tiles.size() / porytiles_legacy::TILES_PNG_WIDTH_IN_TILES));
...
TEST_CASE("emitMetatilesBin should emit metatiles.bin as expected based on settings") {
    ...
    porytiles_legacy::DecompiledTileset decompiled = porytiles_legacy::importLayeredTilesFromPngs(
        ...
        middle, top);
    auto compiled = porytiles_legacy::compile(ctx, porytiles_legacy::CompilerMode::PRIMARY, decompiled, {}, {}, {});

    std::filesystem::path tmpPath = porytiles_legacy::getTmpfilePath(parentDir, "emitMetatilesBin_test.bin");
    std::ofstream outFile{tmpPath};
    porytiles_legacy::emitMetatilesBin(ctx, outFile, *compiled);
    outFile.close();
...
TEST_CASE("emitAttributes should correctly emit metatile attributes") {
    SUBCASE("triple layer metatiles") {
        ...
        auto attributesMap =
            porytiles_legacy::importAttributesFromCsv(ctx, porytiles_legacy::CompilerMode::PRIMARY, behaviorMap,
        ...
        REQUIRE(
            std::filesystem::exists(std::filesystem::path{"resources/doctests/anim_metatiles_2/primary/middle.png"}));
        REQUIRE(std::filesystem::exists(std::filesystem::path{"resources/doctests/anim_metatiles_2/primary/top.png"}));
        png::image<png::rgba_pixel> bottomPrimary{"resources/doctests/anim_metatiles_2/primary/bottom.png"};
        png::image<png::rgba_pixel> middlePrimary{"resources/doctests/anim_metatiles_2/primary/middle.png"};
        png::image<png::rgba_pixel> topPrimary{"resources/doctests/anim_metatiles_2/primary/top.png"};
        porytiles_legacy::DecompiledTileset decompiledPrimary = porytiles_legacy::importLayeredTilesFromPngs(
            ctx, porytiles_legacy::CompilerMode::PRIMARY, attributesMap, bottomPrimary, middlePrimary, topPrimary);
        auto compiled = porytiles_legacy::compile(ctx, porytiles_legacy::CompilerMode::PRIMARY, decompiledPrimary, {}, {}, {});

        std::filesystem::path tmpPath = porytiles_legacy::getTmpfilePath(parentDir, "emitMetatileAttributesBin_test.bin");
        std::ofstream outFile{tmpPath};
        porytiles_legacy::emitAttributes(ctx, outFile, behaviorReverseMap, *compiled);
        outFile.close();
    ...
    SUBCASE("dual layer metatiles") {
        REQUIRE(std::filesystem::exists(std::filesystem::path{"resources/doctests/anim_metatiles_2_dual/primary"}));
        porytiles_legacy::PorytilesContext ctx{};
        std::filesystem::path parentDir = porytiles_legacy::createTmpdir();
        ctx.output.path = parentDir;
        ctx.subcommand = porytiles_legacy::Subcommand::COMPILE_PRIMARY;
        ...
        REQUIRE(std::filesystem::exists(
            std::filesystem::path{"resources/doctests/anim_metatiles_2_dual/primary/middle.png"}));
        REQUIRE(
            std::filesystem::exists(std::filesystem::path{"resources/doctests/anim_metatiles_2_dual/primary/top.png"}));
        png::image<png::rgba_pixel> bottomPrimary{"resources/doctests/anim_metatiles_2_dual/primary/bottom.png"};
        png::image<png::rgba_pixel> middlePrimary{"resources/doctests/anim_metatiles_2_dual/primary/middle.png"};
        png::image<png::rgba_pixel> topPrimary{"resources/doctests/anim_metatiles_2_dual/primary/top.png"};
        porytiles_legacy::DecompiledTileset decompiledPrimary = porytiles_legacy::importLayeredTilesFromPngs(
            ctx, porytiles_legacy::CompilerMode::PRIMARY, attributesMap, bottomPrimary, middlePrimary, topPrimary);
        auto compiled = porytiles_legacy::compile(ctx, porytiles_legacy::CompilerMode::PRIMARY, decompiledPrimary, {}, {}, {});
