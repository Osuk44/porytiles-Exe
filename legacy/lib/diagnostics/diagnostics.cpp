...
using namespace porytiles_legacy;
...
static const DiagTempl W_COLOR_PRECISION_LOSS_NOTE_TEMPL{
    ...
    [](const DiagEngine &eng, const DiagLevel in_flight_level, const std::vector<std::any> &args) -> std::vector<std::string> {
        ...
        const auto tile = AnyCastOrPanic<RGBATile>(args[0], std::source_location::current());
        const auto color = AnyCastOrPanic<std::string>(args[1], std::source_location::current());
        const auto row = AnyCastOrPanic<std::size_t>(args[2], std::source_location::current());
        const auto col = AnyCastOrPanic<std::size_t>(args[3], std::source_location::current());
        ...
        msg.push_back(fmt::format(msg_templ, eng.Bold(tile.prettify()), eng.Bold(color), eng.Bold(col),
                                 eng.Bold(row)));
        auto highlight = BuildTileHighlight(eng, in_flight_level, tile, row, col);
...
static const DiagTempl W_COLOR_PRECISION_LOSS_TEMPL{
    ...
    [](const DiagEngine &eng, const DiagLevel in_flight_level, const std::vector<std::any> &args) -> std::vector<std::string> {
        ...
        const auto tile = AnyCastOrPanic<RGBATile>(args[0], std::source_location::current());
        const auto color = AnyCastOrPanic<std::string>(args[1], std::source_location::current());
        const auto mode = AnyCastOrPanic<std::string>(args[2], std::source_location::current());
        const auto row = AnyCastOrPanic<std::size_t>(args[3], std::source_location::current());
        const auto col = AnyCastOrPanic<std::size_t>(args[4], std::source_location::current());
...
static const DiagTempl W_KEY_FRAME_MISSING_COLORS_NOTE_TEMPL{
    ...
    [](const DiagEngine &eng, const DiagLevel in_flight_level, const std::vector<std::any> &args) -> std::vector<std::string> {
        ...
        const auto missing_colors =
            AnyCastOrPanic<std::vector<RGBA32>>(&args[0], std::source_location::current());
        std::vector<std::string> msg{};
